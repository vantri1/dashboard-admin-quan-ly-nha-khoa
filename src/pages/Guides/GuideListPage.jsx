// src/pages/Posts/GuideListPage.jsx
...
const GuideListPage = () => {
    ...
    const fetchPosts = useCallback(async (page, pageSize, search = '', sortBy = 'id', sortOrder = 'desc') => {
        ...
        try {
            ...
            message.error('Không thể tải dữ liệu bài viết hướng dẫn.');
        } finally {
            setLoading(false);
        }
    }, []);
    ...
    const handleDelete = async (key) => {
        try {
            ...
            message.success(`Đã chuyển bài viết hướng dẫn #${key} vào thùng rác.`);
    ...
    const showBulkDeleteConfirm = () => {
        confirm({
            title: `Bạn có chắc muốn xóa mềm ${selectedRowKeys.length} bài viết hướng dẫn đã chọn?`,
            icon: <ExclamationCircleFilled />,
            content: 'Các bài viết hướng dẫn sẽ được chuyển vào thùng rác và có thể khôi phục sau.',
            ...
            onOk: async () => {
                try {
                    ...
                    message.success(`Đã xóa mềm thành công ${selectedRowKeys.length} bài viết hướng dẫn.`);
                    setSelectedRowKeys([]);
                    fetchPosts(pagination.current, pagination.pageSize, searchTerm, sort.field, sort.order); // Refresh data
                } catch (error) {
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (_, record) => (
                <Space size="middle">
                    <Link to={`/guides/edit/${record.id}`}>
                        <Button type="text" icon={<EditOutlined />} />
                    </Link>
                    <Popconfirm
                        title="Bạn có chắc muốn xóa bài viết hướng dẫn này?"
    ...
    return (
        <div className="space-y-4">
            <Title level={3}>Danh sách Bài viết hướng dẫn</Title>
            <div className="flex justify-between items-center">
                <Input.Search
                    placeholder="Tìm theo tiêu đề bài hướng dẫn..."
                ...
                />
                <Link to="/guides/add">
                    <Button type="primary" icon={<PlusOutlined />}>Thêm bài viết hướng dẫn mới</Button>
                </Link>
            </div>
            {hasSelected && (
                <Alert
